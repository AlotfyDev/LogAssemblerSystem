...
/*
 * File responsibility:
 *   TPolicyRealizerId identifies a policy realizer implementation or realizer
 *   interface participant. The identifier does not imply the realizer is
 *   available, authorized, runtime-ready, or allowed to invoke any target.
 */
struct TPolicyRealizerId final
{
    std::uint64_t value{0};

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }
    [[nodiscard]] constexpr explicit operator bool() const noexcept { return IsValid(); }
    [[nodiscard]] friend constexpr bool operator==(TPolicyRealizerId lhs, TPolicyRealizerId rhs) noexcept { return lhs.value == rhs.value; }
    [[nodiscard]] friend constexpr bool operator!=(TPolicyRealizerId lhs, TPolicyRealizerId rhs) noexcept { return !(lhs == rhs); }
    [[nodiscard]] friend constexpr bool operator<(TPolicyRealizerId lhs, TPolicyRealizerId rhs) noexcept { return lhs.value < rhs.value; }
};
