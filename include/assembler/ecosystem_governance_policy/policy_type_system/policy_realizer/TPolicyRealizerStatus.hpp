...
/*
 * File responsibility:
 *   TPolicyRealizerStatus defines lifecycle/readiness states for a realizer
 *   interface participant. These states describe availability of the interface
 *   participant only. They do not mean a policy has been executed.
 */
enum class TPolicyRealizerStatus : std::uint8_t
{
    unknown = 0,
    unavailable = 1,
    discovered = 2,
...
[[nodiscard]] constexpr bool IsReady(TPolicyRealizerStatus status) noexcept
{
    return status == TPolicyRealizerStatus::ready ||
           status == TPolicyRealizerStatus::ready_with_warnings;
}

[[nodiscard]] constexpr bool IsBlocking(TPolicyRealizerStatus status) noexcept
{
    return status == TPolicyRealizerStatus::unavailable ||
           status == TPolicyRealizerStatus::blocked ||
           status == TPolicyRealizerStatus::forbidden ||
           status == TPolicyRealizerStatus::retired;
}
