...
/*
 * File responsibility:
 *   TPolicyRealizerIssue is a diagnostic carrier for interface-level problems:
 *   missing plan, missing readiness report, unsupported capability, blocked
 *   contract, invalid state, or forbidden invocation attempt. It is not a log
 *   backend, not telemetry, and not a recovery mechanism.
 */
struct TPolicyRealizerIssue final
{
    std::string code{};
    std::string message{};
    bool blocking{false};
    ...
    [[nodiscard]] bool IsBlocking() const noexcept { return blocking || forbidden; }
    [[nodiscard]] bool IsClear() const noexcept { return code.empty() && message.empty() && !blocking && !forbidden; }
};
