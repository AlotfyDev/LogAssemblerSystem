...
#include "assembler/ecosystem_governance/policy_type_system/policy_realizer/TPolicyRealizerIssue.hpp"
...
/*
 * File responsibility:
 *   TPolicyRealizerResult describes the outcome of interacting with the W21
 *   interface boundary. It can report acceptance, rejection, deferral, dry-run
 *   result, planning result, or forbidden status. It must not claim target
 *   mutation, downstream delivery, or external success.
 */
enum class TPolicyRealizerResultKind : std::uint8_t
{
    unknown = 0,
    accepted = 1,
    rejected = 2,
...
[[nodiscard]] constexpr bool IsPositive(TPolicyRealizerResultKind kind) noexcept
{
    return kind == TPolicyRealizerResultKind::accepted ||
           kind == TPolicyRealizerResultKind::dry_run_passed ||
           kind == TPolicyRealizerResultKind::plan_consumed;
}

[[nodiscard]] constexpr bool IsBlocking(TPolicyRealizerResultKind kind) noexcept
{
    return kind == TPolicyRealizerResultKind::rejected ||
           kind == TPolicyRealizerResultKind::dry_run_failed ||
           kind == TPolicyRealizerResultKind::readiness_missing ||
           kind == TPolicyRealizerResultKind::unsupported_capability ||
           kind == TPolicyRealizerResultKind::forbidden ||
           kind == TPolicyRealizerResultKind::failed;
}

struct TPolicyRealizerResult final
{
    TPolicyRealizerResultKind kind{TPolicyRealizerResultKind::unknown};
    TPolicyRealizationTrace trace{};
    std::vector<TPolicyRealizerIssue> issues{};
    std::string note{};
    ...
    [[nodiscard]] bool HasBlockingIssue() const noexcept
    {
        return policy_type_system::IsBlocking(kind) ||
               std::any_of(issues.begin(), issues.end(), [](const TPolicyRealizerIssue& issue) { return issue.IsBlocking(); });
    }
};
