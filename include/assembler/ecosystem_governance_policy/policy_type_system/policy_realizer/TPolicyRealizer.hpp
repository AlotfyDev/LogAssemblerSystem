...
#include "assembler/ecosystem_governance/policy_type_system/policy_realizer/TPolicyRealizerResult.hpp"
#include "assembler/ecosystem_governance/policy_type_system/policy_realizer/TPolicyRealizerView.hpp"
...
/*
 * File responsibility:
 *   TPolicyRealizer defines the narrow interface boundary for future policy
 *   realization. It is intentionally conservative: implementations may expose
 *   a view and may accept/inspect a pre-execution runtime plan, but the
 *   interface itself does not define target invocation semantics, scheduler
 *   behavior, persistence, or external side effects. Concrete runtime behavior
 *   must be introduced only by later waves and only after compliance gates.
 */
class TPolicyRealizer
{
public:
    virtual ~TPolicyRealizer() = default;

    [[nodiscard]] virtual TPolicyRealizerView View() const = 0;

    [[nodiscard]] virtual bool CanAccept(const TPolicyRuntimePlan& plan,
                                         const TPolicyRuntimeReadinessReport& readiness) const = 0;

    [[nodiscard]] virtual TPolicyRealizerResult InspectPlan(const TPolicyRuntimePlan& plan,
                                                            const TPolicyRuntimeReadinessReport& readiness) const = 0;
};
