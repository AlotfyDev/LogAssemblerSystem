...
/*
 * File: TPolicyRealizationEngineId.hpp
 ...
 *
 * File responsibility:
 *   TPolicyRealizationEngineId is the stable identity carrier for a policy realization engine skeleton instance. It is only identity; it does not imply runtime ownership, scheduling authority, target invocation, or execution capability.
 */
struct TPolicyRealizationEngineId final
{
    std::uint64_t value{0};
    [[nodiscard]] bool IsValid() const noexcept { return value != 0; }
    [[nodiscard]] friend bool operator==(TPolicyRealizationEngineId a, TPolicyRealizationEngineId b) noexcept { return a.value == b.value; }
    [[nodiscard]] friend bool operator!=(TPolicyRealizationEngineId a, TPolicyRealizationEngineId b) noexcept { return !(a == b); }
};
