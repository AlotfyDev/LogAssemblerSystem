...
/*
 * File: TPolicyRealizationEngineIssue.hpp
 ...
 *
 * File responsibility:
 *   TPolicyRealizationEngineIssue records why the engine skeleton cannot inspect, accept, or forward a pre-execution plan. Issues are evidence and diagnostics only; they do not repair, execute, roll back, or mutate anything.
 */
enum class TPolicyRealizationEngineIssueKind : std::uint8_t
{
    unknown = 0,
    missing_engine_id = 1,
    engine_not_configured = 2,
    missing_execution_plan = 3,
    execution_plan_not_ready = 4,
    missing_authority_evidence = 5,
    missing_trace = 6,
    step_budget_exceeded = 7,
    blocking_plan_issue = 8,
    forbidden_plan = 9,
    unsupported_realizer = 10,
    direct_target_invocation_attempt = 11
};

enum class TPolicyRealizationEngineIssueSeverity : std::uint8_t
{
    info = 0,
    warning = 1,
    blocking = 2,
...
struct TPolicyRealizationEngineIssue final
{
    TPolicyRealizationEngineIssueKind kind{TPolicyRealizationEngineIssueKind::unknown};
    TPolicyRealizationEngineIssueSeverity severity{TPolicyRealizationEngineIssueSeverity::info};
    std::string message{};

    [[nodiscard]] bool IsBlocking() const noexcept
    {
        return severity == TPolicyRealizationEngineIssueSeverity::blocking ||
               severity == TPolicyRealizationEngineIssueSeverity::critical ||
               severity == TPolicyRealizationEngineIssueSeverity::forbidden;
    }

    [[nodiscard]] static TPolicyRealizationEngineIssue Blocking(TPolicyRealizationEngineIssueKind k, std::string m)
    {
        return TPolicyRealizationEngineIssue{k, TPolicyRealizationEngineIssueSeverity::blocking, std::move(m)};
    }

    [[nodiscard]] static TPolicyRealizationEngineIssue Forbidden(TPolicyRealizationEngineIssueKind k, std::string m)
    {
        return TPolicyRealizationEngineIssue{k, TPolicyRealizationEngineIssueSeverity::forbidden, std::move(m)};
    }
};
