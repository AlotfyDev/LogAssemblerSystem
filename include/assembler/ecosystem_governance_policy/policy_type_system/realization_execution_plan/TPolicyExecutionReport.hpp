...
/*
 * File: TPolicyExecutionReport.hpp
 ...
 *
 * File responsibility:
 *   TPolicyExecutionReport records validation of a realization execution plan. It reports whether the plan is draft, ready, blocked, failed, or forbidden. It is evidence, not execution, and must not claim target mutation or downstream success.
 */
struct TPolicyExecutionReport final
{
    TPolicyExecutionPlanId plan_id{};
    TPolicyExecutionPlanStatus status{TPolicyExecutionPlanStatus::unknown};
    ...
    {
        return policy_type_system::IsBlocking(status) || std::any_of(issues.begin(), issues.end(), [](const TPolicyExecutionIssue& i) { return i.IsBlocking(); });
    }

    [[nodiscard]] bool MayProceedToFutureExecutorInspection() const noexcept
    {
        return policy_type_system::IsPositive(status) && !HasBlockingIssue();
    }
};
