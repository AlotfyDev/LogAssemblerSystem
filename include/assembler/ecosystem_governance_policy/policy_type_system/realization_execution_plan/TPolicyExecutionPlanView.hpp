...
/*
 * File: TPolicyExecutionPlanView.hpp
 ...
 *
 * File responsibility:
 *   TPolicyExecutionPlanView is the read-only projection of a W23 execution plan. It exposes identity, session, runtime-plan, realizer, status, step count, and trace id without exposing mutation or execution behavior.
 */
struct TPolicyExecutionPlanView final
{
    ...
    {
        return plan_id.IsValid() && session_id.IsValid() && policy_type_system::IsPositive(status);
    }
};
