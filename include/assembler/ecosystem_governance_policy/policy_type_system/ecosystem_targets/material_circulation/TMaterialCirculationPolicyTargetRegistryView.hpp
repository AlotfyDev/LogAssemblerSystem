...
#include "assembler/ecosystem_governance/policy_type_system/ecosystem_targets/material_circulation/TMaterialCirculationPolicyTargetCapabilityProfile.hpp"
...
/*
 * File responsibility:
 *   TMaterialCirculationPolicyTargetRegistryView is an in-memory view of the
 *   W26 material-circulation policy-target descriptors and capability
 *   profiles. It is not the policy registry, a service locator, target locator,
 *   or persistence layer.
 */
struct TMaterialCirculationPolicyTargetRegistryView final
{
    std::vector<TMaterialCirculationPolicyTargetCapabilityProfile> targets{};

    void Add(TMaterialCirculationPolicyTargetCapabilityProfile profile)
    {
    ...
    [[nodiscard]] const TMaterialCirculationPolicyTargetCapabilityProfile* Find(
        TMaterialCirculationPolicyTargetKind kind) const noexcept
    {
        auto it = std::find_if(targets.begin(), targets.end(), [&](const auto& item) {
    ...
    [[nodiscard]] bool Has(TMaterialCirculationPolicyTargetKind kind) const noexcept
    {
        return Find(kind) != nullptr;
    }

    [[nodiscard]] bool IsCompleteForCoreMaterialTargets() const noexcept
    {
        return Has(TMaterialCirculationPolicyTargetKind::admission) &&
               Has(TMaterialCirculationPolicyTargetKind::slot) &&
               Has(TMaterialCirculationPolicyTargetKind::slots_container) &&
               Has(TMaterialCirculationPolicyTargetKind::waiting_list) &&
               Has(TMaterialCirculationPolicyTargetKind::receiving_round_manager) &&
               Has(TMaterialCirculationPolicyTargetKind::delivery_round_manager) &&
               Has(TMaterialCirculationPolicyTargetKind::dispatcher);
    }
};
