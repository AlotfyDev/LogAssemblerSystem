...
/*
 * File responsibility:
 *   TMaterialCirculationPolicyTargetIntegrationReport records structural
 *   findings for the W26 material-circulation target slice. It is diagnostic
 *   evidence for integration completeness only; it is not runtime telemetry,
 *   assignment activation, or execution output.
 */
enum class TMaterialCirculationPolicyTargetIntegrationIssueSeverity : std::uint8_t
{
    info = 0,
    warning = 1,
    blocking = 2,
...
enum class TMaterialCirculationPolicyTargetIntegrationIssueKind : std::uint8_t
{
    unknown = 0,
    missing_target = 1,
...
struct TMaterialCirculationPolicyTargetIntegrationIssue final
{
    TMaterialCirculationPolicyTargetIntegrationIssueSeverity severity{TMaterialCirculationPolicyTargetIntegrationIssueSeverity::info};
    TMaterialCirculationPolicyTargetIntegrationIssueKind kind{TMaterialCirculationPolicyTargetIntegrationIssueKind::unknown};
    std::string message{};

    [[nodiscard]] bool IsBlocking() const noexcept
    {
        return severity == TMaterialCirculationPolicyTargetIntegrationIssueSeverity::blocking ||
               severity == TMaterialCirculationPolicyTargetIntegrationIssueSeverity::critical;
    }
};

struct TMaterialCirculationPolicyTargetIntegrationReport final
{
    std::vector<TMaterialCirculationPolicyTargetIntegrationIssue> issues{};
    std::size_t described_targets{0};
    std::size_t compatibility_rules{0};

    void AddIssue(TMaterialCirculationPolicyTargetIntegrationIssueSeverity severity,
                  TMaterialCirculationPolicyTargetIntegrationIssueKind kind,
                  std::string message)
    {
        issues.push_back(TMaterialCirculationPolicyTargetIntegrationIssue{severity, kind, std::move(message)});
    }
