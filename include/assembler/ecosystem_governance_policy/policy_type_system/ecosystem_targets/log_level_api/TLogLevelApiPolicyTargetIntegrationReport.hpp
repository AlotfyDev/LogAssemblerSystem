...
/*
 * File: TLogLevelApiPolicyTargetIntegrationReport.hpp
 ...
 * File responsibility:
 *   TLogLevelApiPolicyTargetIntegrationReport records the non-executing integration health of log_level_api policy target descriptors, capability profiles, and compatibility rows. It reports readiness of descriptors only; it does not activate or realize policies.
 */

enum class TLogLevelApiPolicyTargetIntegrationIssueSeverity : std::uint8_t
{
    info = 0,
    warning = 1,
    blocking = 2,
...
enum class TLogLevelApiPolicyTargetIntegrationIssueKind : std::uint8_t
{
    none = 0,
    missing_target_descriptor = 1,
    invalid_target_descriptor = 2,
    missing_capability_profile = 3,
    missing_compatibility_rules = 4,
    compatibility_failed = 5,
    forbidden_runtime_behavior = 6,
    custom = 255
};

struct TLogLevelApiPolicyTargetIntegrationIssue final
{
    TLogLevelApiPolicyTargetIntegrationIssueKind kind{TLogLevelApiPolicyTargetIntegrationIssueKind::none};
    TLogLevelApiPolicyTargetIntegrationIssueSeverity severity{TLogLevelApiPolicyTargetIntegrationIssueSeverity::info};
    std::string message{};

    [[nodiscard]] bool IsBlocking() const noexcept
    {
        return severity == TLogLevelApiPolicyTargetIntegrationIssueSeverity::blocking ||
               severity == TLogLevelApiPolicyTargetIntegrationIssueSeverity::forbidden;
    }
};

struct TLogLevelApiPolicyTargetIntegrationReport final
{
    std::vector<TLogLevelApiPolicyTargetIntegrationIssue> issues{};
    std::uint64_t target_count{0};
    std::uint64_t valid_target_count{0};
    std::uint64_t profile_count{0};
    std::uint64_t compatibility_rule_count{0};
    std::string summary{};

    void AddIssue(TLogLevelApiPolicyTargetIntegrationIssue issue) { issues.push_back(std::move(issue)); }
    ...
    {
        return !HasBlockingIssue() && target_count != 0 && valid_target_count == target_count && profile_count == target_count;
    }
};
