...
#include "assembler/ecosystem_governance/policy_type_system/ecosystem_targets/system_wide/TSystemWidePolicyIntegrationIssue.hpp"
...
/*
 * File responsibility:
 *   TSystemWidePolicyIntegrationReport summarizes W29 system-wide integration
 *   readiness. It reports whether the descriptive target slices are present,
 *   not whether any policy has been assigned, realized, or executed.
 */
struct TSystemWidePolicyIntegrationReport final
{
    std::size_t target_slices_present{0};
    std::size_t target_slices_expected{4};
    std::size_t plan_steps{0};
    std::vector<TSystemWidePolicyIntegrationIssue> issues{};
    std::string summary{};

    void AddIssue(TSystemWidePolicyIntegrationIssue issue)
    {
    ...
    [[nodiscard]] bool IsPositive() const noexcept
    {
        ...
        return target_slices_present >= target_slices_expected;
    }
};
