...
/*
 * File responsibility:
 *   TSystemWidePolicyIntegrationPlanStep names a planned non-executing
 *   integration checkpoint. A step may mention registry lookup, assignment
 *   inspection, readiness review, or realizer eligibility, but it must not
 *   perform any of those operations against live runtime targets.
 */
enum class TSystemWidePolicyIntegrationPlanStepKind : std::uint8_t
{
    unknown = 0,
    locate_published_policy = 1,
    inspect_assignment = 2,
    resolve_target_slice = 3,
    inspect_runtime_readiness = 4,
    inspect_runtime_plan = 5,
    inspect_realizer_surface = 6,
    produce_integration_report = 7
};

struct TSystemWidePolicyIntegrationPlanStep final
{
    TSystemWidePolicyIntegrationPlanStepKind kind{TSystemWidePolicyIntegrationPlanStepKind::unknown};
    std::string description{};

    [[nodiscard]] bool IsKnown() const noexcept
    {
        return kind != TSystemWidePolicyIntegrationPlanStepKind::unknown;
    }
};
