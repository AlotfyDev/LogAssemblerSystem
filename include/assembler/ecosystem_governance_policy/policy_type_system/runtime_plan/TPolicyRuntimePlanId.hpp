 ...
 * File responsibility:
 *   TPolicyRuntimePlanId defines stable identity for runtime plan artifacts.
 *   It is a value identifier only; it does not own the plan, execute the plan,
 *   invoke a target, or imply that runtime realization has occurred.
 */

struct TPolicyRuntimePlanId final
{
    std::uint64_t value{0};

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }
    [[nodiscard]] constexpr explicit operator bool() const noexcept { return IsValid(); }
    [[nodiscard]] friend constexpr bool operator==(TPolicyRuntimePlanId lhs, TPolicyRuntimePlanId rhs) noexcept { return lhs.value == rhs.value; }
    [[nodiscard]] friend constexpr bool operator!=(TPolicyRuntimePlanId lhs, TPolicyRuntimePlanId rhs) noexcept { return !(lhs == rhs); }
    [[nodiscard]] friend constexpr bool operator<(TPolicyRuntimePlanId lhs, TPolicyRuntimePlanId rhs) noexcept { return lhs.value < rhs.value; }
};
