...
/*
 * File: TPolicyRuntimePlanStatus.hpp
 ...
 * File responsibility:
 *   TPolicyRuntimePlanStatus defines non-executing lifecycle states for a
 *   runtime plan artifact. These states describe whether a plan is draft,
 *   ready, blocked, forbidden, cancelled, or expired. They do not mean the
 *   policy has been realized or executed.
 */

enum class TPolicyRuntimePlanStatus : std::uint8_t
{
    unknown = 0,
    draft = 1,
    plan_requested = 2,
    plan_ready = 3,
    plan_ready_with_warnings = 4,
...
[[nodiscard]] constexpr bool IsPositive(TPolicyRuntimePlanStatus status) noexcept
{
    return status == TPolicyRuntimePlanStatus::plan_ready ||
           status == TPolicyRuntimePlanStatus::plan_ready_with_warnings ||
           status == TPolicyRuntimePlanStatus::conditionally_ready;
}

[[nodiscard]] constexpr bool IsBlocking(TPolicyRuntimePlanStatus status) noexcept
{
    return status == TPolicyRuntimePlanStatus::blocked ||
           status == TPolicyRuntimePlanStatus::forbidden ||
           status == TPolicyRuntimePlanStatus::cancelled ||
           status == TPolicyRuntimePlanStatus::expired;
}
