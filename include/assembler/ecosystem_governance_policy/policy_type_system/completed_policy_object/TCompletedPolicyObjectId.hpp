 ...
 * File: TCompletedPolicyObjectId.hpp
 *
 * Architectural role:
 *   Defines the primitive identity for a completed policy object. A completed policy object is the product of governance semantics plus target-side realization mapping, not a host assignment.
 *
...
struct TCompletedPolicyObjectId
{
    std::string value;

    TCompletedPolicyObjectId() = default;
    explicit TCompletedPolicyObjectId(std::string v) : value(std::move(v)) {}

    [[nodiscard]] bool empty() const noexcept { return value.empty(); }
    [[nodiscard]] explicit operator bool() const noexcept { return !empty(); }

    friend bool operator==(const TCompletedPolicyObjectId& a, const TCompletedPolicyObjectId& b) noexcept
