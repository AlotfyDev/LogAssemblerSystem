...
/*
 * File responsibility:
 *   Report returned by an audit export boundary description or admission check.
 *   It collects descriptor, record/snapshot references, admission status, and
 *   non-executing issues for diagnostics and compliance review.
 */
struct TPolicyAuditExportBoundaryReport final
{
    std::string report_id{};
    TPolicyAuditExportBoundaryDescriptor descriptor{};
    TPolicyAuditExportAdmission admission{};
    std::vector<TPolicyAuditExportRecordRef> records{};
    std::vector<TPolicyAuditExportSnapshotRef> snapshots{};
    std::vector<std::string> issues{};

    bool ok() const noexcept { return issues.empty() && admission.admitted; }
};
