...
/*
 * File responsibility:
 *   Describes whether a governance record or snapshot is admitted to the audit
 *   export boundary. Admission is descriptive; it does not export anything.
 */
struct TPolicyAuditExportAdmission final
{
    bool admitted{false};
    std::string reason{};
    std::vector<std::string> issues{};

    static TPolicyAuditExportAdmission accept(std::string why = {})
    {
        TPolicyAuditExportAdmission a{};
        a.admitted = true;
        a.reason = std::move(why);
        return a;
    }

    static TPolicyAuditExportAdmission reject(std::string why)
    {
        TPolicyAuditExportAdmission a{};
        a.admitted = false;
        a.reason = std::move(why);
        a.issues.push_back(a.reason);
        return a;
    }
};
