...
#include "assembler/ecosystem_governance/policy_persistence/TPolicyPersistenceBoundaryId.hpp"
#include "assembler/ecosystem_governance/policy_persistence/TPolicyPersistenceMediumKind.hpp"
#include <string>
...
/*
 * File responsibility:
 *   Describes a persistence boundary, its medium category, visibility,
 *   retention class, and anti-collapse notes. This descriptor is not a concrete
 *   storage adapter and does not implement persistence.
 */
struct TPolicyPersistenceBoundaryDescriptor final
{
    TPolicyPersistenceBoundaryId id{};
    std::string name{"policy_persistence_boundary"};
    TPolicyPersistenceMediumKind medium{TPolicyPersistenceMediumKind::unspecified};
    std::string visibility{"governance_review"};
    std::string retention_class{"unspecified"};
    bool production_visible{false};

    bool valid() const noexcept
    {
        return !id.value.empty() && medium != TPolicyPersistenceMediumKind::forbidden;
    }
};
