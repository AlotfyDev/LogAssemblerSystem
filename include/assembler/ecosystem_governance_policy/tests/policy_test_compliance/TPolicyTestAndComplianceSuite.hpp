...
#include <cstddef>
#include <utility>
#include "TPolicyComplianceSuiteId.hpp"
#include "TPolicyComplianceSuiteReport.hpp"
...
/* Top-level non-executing policy test and compliance suite descriptor. */
struct TPolicyTestAndComplianceSuite{TPolicyComplianceSuiteId suite_id;std::string title;std::vector<TPolicyComplianceTestCase> cases;TPolicyComplianceSuiteReport report;std::vector<std::string> covered_waves;std::vector<std::string> deferred_decisions;[[nodiscard]] std::size_t case_count() const noexcept{return cases.size();}[[nodiscard]] bool empty() const noexcept{return cases.empty();}void add_case(TPolicyComplianceTestCase test_case){cases.push_back(std::move(test_case));report.planned_cases=cases.size();}};
