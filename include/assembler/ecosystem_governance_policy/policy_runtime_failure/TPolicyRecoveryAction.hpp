...
#include "assembler/ecosystem_governance/policy_runtime_failure/TPolicyRecoveryActionKind.hpp"
#include <string>
#include <utility>
...
/*
 * File responsibility:
 *   Describes one proposed recovery action. It is a request descriptor only.
 *   It must not execute the described action or imply authority has been granted.
 */
struct TPolicyRecoveryAction final
{
    std::string action_id{};
    TPolicyRecoveryActionKind kind{TPolicyRecoveryActionKind::none};
    ...
    {
        return !action_id.empty() && kind != TPolicyRecoveryActionKind::none;
    }

    [[nodiscard]] bool may_be_considered_without_execution() const noexcept
    {
        return valid() && execution_forbidden;
    }

    static TPolicyRecoveryAction propose(std::string id,
                                         TPolicyRecoveryActionKind action_kind,
                                         std::string subject = {},
                                         std::string why = {})
    {
        TPolicyRecoveryAction action{};
        action.action_id = std::move(id);
        action.kind = action_kind;
        action.subject_ref = std::move(subject);
        action.reason = std::move(why);
        return action;
    }
};
