...
/*
 * File responsibility:
 *   Describes whether a policy, assignment, runtime plan, session, target
 *   descriptor, or engine-facing context has been isolated for review. The type
 *   does not quarantine anything by itself.
 */
struct TPolicyQuarantineState final
{
    ...
    {
        return !quarantined || !reason.empty();
    }

    [[nodiscard]] bool blocks_runtime_progression() const noexcept
    {
        return quarantined;
    }

    static TPolicyQuarantineState clear(std::string subject = {})
    {
        TPolicyQuarantineState state{};
        state.subject_ref = std::move(subject);
        return state;
    }

    static TPolicyQuarantineState isolate(std::string subject, std::string why)
    {
        TPolicyQuarantineState state{};
        state.quarantined = true;
        state.subject_ref = std::move(subject);
        state.reason = std::move(why);
        return state;
    }
};
