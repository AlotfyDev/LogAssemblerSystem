...
struct TLogApiContentRefLeaseId final {
    std::uint64_t value{0};

    constexpr TLogApiContentRefLeaseId() = default;
    constexpr explicit TLogApiContentRefLeaseId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-content-ref-lease-" + std::to_string(value);
    }
};
