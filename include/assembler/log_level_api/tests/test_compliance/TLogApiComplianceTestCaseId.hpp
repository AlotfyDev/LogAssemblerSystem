...
struct TLogApiComplianceTestCaseId final {
    std::uint64_t value{0};

    constexpr TLogApiComplianceTestCaseId() = default;
    constexpr explicit TLogApiComplianceTestCaseId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-compliance-test-case-" + std::to_string(value);
    }
};
