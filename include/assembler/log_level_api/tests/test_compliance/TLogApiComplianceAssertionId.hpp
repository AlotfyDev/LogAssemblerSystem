...
struct TLogApiComplianceAssertionId final {
    std::uint64_t value{0};

    constexpr TLogApiComplianceAssertionId() = default;
    constexpr explicit TLogApiComplianceAssertionId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-compliance-assertion-" + std::to_string(value);
    }
};
