...
/*
    ...
    This class represents the validation-required assembly family.
    It requires validation evidence to be present and accepted.
*/
class TValidatorComposedLogApiEnvelopeAssembler final {
public:
    [[nodiscard]] static TLogApiEnvelopeAssemblyResult Assemble(
        ...
        if (profile.family != assembler::log_level_api::api_configurator::TLogApiAssemblerFamilyKind::ValidatorComposedEnvelopeAssembler) {
            result.issues.push_back({"LOGAPI_ASSEMBLY_WRONG_FAMILY", "Validator-composed assembler requires ValidatorComposedEnvelopeAssembler family.", true});
            return result;
        }
        ...
        if (!input.validation_present || !input.validation.IsAccepted()) {
            result.issues.push_back({"LOGAPI_ASSEMBLY_VALIDATION_MISSING_OR_REJECTED", "Validator-composed assembly requires accepted validation evidence.", true});
            return result;
        }
        ...
        envelope.header.status = assembler::log_level_api::envelope::TLogApiEnvelopeStatus::Validated;
        ...
        envelope.validation.present = true;
        envelope.validation.validation = input.validation;

        result.envelope_result.envelope = envelope;

        if (!result.envelope_result.envelope.IsComplete()) {
            result.envelope_result.issues.push_back({"LOGAPI_ENVELOPE_INCOMPLETE", "Validator-composed assembly produced an incomplete envelope.", true});
        }
