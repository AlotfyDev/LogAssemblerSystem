...
struct TLogApiEnvelopeAssemblerId final {
    std::uint64_t value{0};

    constexpr TLogApiEnvelopeAssemblerId() = default;
    constexpr explicit TLogApiEnvelopeAssemblerId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-envelope-assembler-" + std::to_string(value);
    }
};
