...
struct TLogApiFacadeInputResult final {
    TLogApiContentInputStatus status{TLogApiContentInputStatus::Unknown};
    TLogApiFacadeAcceptedContent accepted_content{};
    std::vector<TLogApiFacadeIssue> issues{};
    ...
    [[nodiscard]] bool IsAccepted() const noexcept {
        return status == TLogApiContentInputStatus::Accepted
            && accepted_content.IsValid()
            && !HasBlockingIssue();
    }
};
