...
struct TLogApiConfigurationId final {
    std::uint64_t value{0};

    constexpr TLogApiConfigurationId() = default;
    constexpr explicit TLogApiConfigurationId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-config-" + std::to_string(value);
    }
};
