...
struct TLogApiCStructSchemaPackId final {
    std::uint64_t value{0};

    constexpr TLogApiCStructSchemaPackId() = default;
    constexpr explicit TLogApiCStructSchemaPackId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-cstruct-schema-pack-" + std::to_string(value);
    }
};
