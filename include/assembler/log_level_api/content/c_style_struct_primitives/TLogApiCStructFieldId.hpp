...
struct TLogApiCStructFieldId final {
    std::uint64_t value{0};

    constexpr TLogApiCStructFieldId() = default;
    constexpr explicit TLogApiCStructFieldId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-cstruct-field-" + std::to_string(value);
    }
};
