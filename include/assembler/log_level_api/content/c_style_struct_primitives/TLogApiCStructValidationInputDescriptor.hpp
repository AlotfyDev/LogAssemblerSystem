...
struct TLogApiCStructValidationInputDescriptor final {
    const TLogApiCStructLevelSchemaDescriptor* schema{nullptr};
    ...
    [[nodiscard]] std::size_t RequiredFieldCount() const noexcept {
        return IsReady() ? schema->layout.RequiredCount() : 0;
    }

    [[nodiscard]] std::size_t DefaultedFieldCount() const noexcept {
        return IsReady() ? schema->layout.DefaultedCount() : 0;
    }

    [[nodiscard]] std::uint32_t TotalSizeBytes() const noexcept {
        return IsReady() ? schema->layout.total_size_bytes : 0;
    }
};
