...
struct TLogApiJsonFieldId final {
    std::uint64_t value{0};

    constexpr TLogApiJsonFieldId() = default;
    constexpr explicit TLogApiJsonFieldId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-json-field-" + std::to_string(value);
    }
};
