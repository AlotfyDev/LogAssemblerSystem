...
struct TLogApiJsonSchemaId final {
    std::uint64_t value{0};

    constexpr TLogApiJsonSchemaId() = default;
    constexpr explicit TLogApiJsonSchemaId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-json-schema-" + std::to_string(value);
    }
};
