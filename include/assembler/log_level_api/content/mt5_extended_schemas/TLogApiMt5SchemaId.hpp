...
struct TLogApiMt5SchemaId final {
    std::uint64_t value{0};

    constexpr TLogApiMt5SchemaId() = default;
    constexpr explicit TLogApiMt5SchemaId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-mt5-schema-" + std::to_string(value);
    }
};
