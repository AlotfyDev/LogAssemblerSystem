...
struct TLogApiMt5FieldId final {
    std::uint64_t value{0};

    constexpr TLogApiMt5FieldId() = default;
    constexpr explicit TLogApiMt5FieldId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-mt5-field-" + std::to_string(value);
    }
};
