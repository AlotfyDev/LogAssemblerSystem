...
struct TLogApiMt5SchemaPackId final {
    std::uint64_t value{0};

    constexpr TLogApiMt5SchemaPackId() = default;
    constexpr explicit TLogApiMt5SchemaPackId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-mt5-schema-pack-" + std::to_string(value);
    }
};
