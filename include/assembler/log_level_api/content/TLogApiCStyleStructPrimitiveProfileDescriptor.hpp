...
/*
    Default-friendly descriptor for the C-style struct primitive schema path.

    It does not define concrete struct layouts, offsets, padding, alignment,
    endianness, or cross-language mappings. Those belong to later default
    C-style struct primitive schema waves.
*/
struct TLogApiCStyleStructPrimitiveProfileDescriptor final {
    std::string profile_name{"default.c_style_struct_primitives.minimal"};
    std::string schema_family_name{"c_style_struct_primitives"};
    ...
    [[nodiscard]] TLogApiContentSchemaPathDescriptor ToPathDescriptor() const {
        ...
        d.kind = assembler::log_level_api::api_configurator::TLogApiSchemaPathKind::CStyleStructPrimitives;
