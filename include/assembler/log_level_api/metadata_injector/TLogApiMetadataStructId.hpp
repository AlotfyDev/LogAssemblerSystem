...
struct TLogApiMetadataStructId final {
    std::uint64_t value{0};

    constexpr TLogApiMetadataStructId() = default;
    constexpr explicit TLogApiMetadataStructId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-metadata-struct-" + std::to_string(value);
    }
};
