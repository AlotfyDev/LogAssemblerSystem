...
struct TLogApiMetadataProfileLookupResult final {
    const TLogApiMetadataProfileDescriptor* profile{nullptr};

    [[nodiscard]] bool FoundUsable() const noexcept {
        return profile != nullptr && profile->IsUsable();
    }
};
