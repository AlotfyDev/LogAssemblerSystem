...
struct TLogApiTimestampStabilizationRuleId final {
    std::uint64_t value{0};

    constexpr TLogApiTimestampStabilizationRuleId() = default;
    constexpr explicit TLogApiTimestampStabilizationRuleId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-timestamp-stabilization-rule-" + std::to_string(value);
    }
};
