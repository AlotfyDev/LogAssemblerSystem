...
struct TLogApiTimestampSafePointFlag final {
    TLogApiTimestampSafePointKind kind{TLogApiTimestampSafePointKind::Unknown};
    bool raised{false};

    [[nodiscard]] bool IsRaised(TLogApiTimestampSafePointKind expected) const noexcept {
        return raised && kind == expected;
    }
};
