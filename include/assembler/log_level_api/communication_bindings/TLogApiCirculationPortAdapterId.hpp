...
struct TLogApiCirculationPortAdapterId final {
    std::uint64_t value{0};

    constexpr TLogApiCirculationPortAdapterId() = default;
    constexpr explicit TLogApiCirculationPortAdapterId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "circulation-intake-port-adapter-" + std::to_string(value);
    }
};
