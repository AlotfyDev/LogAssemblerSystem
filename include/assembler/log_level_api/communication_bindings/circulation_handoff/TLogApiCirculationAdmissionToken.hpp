...
/*
    Admission token issued by the contract layer.

    This is not proof of circulation placement. It only means the request is
    well-formed for future circulation admission.
*/
struct TLogApiCirculationAdmissionToken final {
    std::uint64_t value{0};

    constexpr TLogApiCirculationAdmissionToken() = default;
    constexpr explicit TLogApiCirculationAdmissionToken(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-circulation-admission-token-" + std::to_string(value);
    }
};
