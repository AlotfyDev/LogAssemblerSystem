...
#include "assembler/log_level_api/envelope/TLogApiEnvelope.hpp"

namespace assembler::log_level_api::communication_bindings {

/*
    Handoff request carrier.

    It carries an already assembled envelope toward the binding. It does not
    execute circulation admission.
*/
struct TLogApiCirculationHandoffRequest final {
    const assembler::log_level_api::envelope::TLogApiEnvelope* envelope{nullptr};

    [[nodiscard]] bool HasEnvelope() const noexcept {
        return envelope != nullptr && envelope->IsComplete();
    }
};
