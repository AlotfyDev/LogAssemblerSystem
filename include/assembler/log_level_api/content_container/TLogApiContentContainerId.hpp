...
struct TLogApiContentContainerId final {
    std::uint64_t value{0};

    constexpr TLogApiContentContainerId() = default;
    constexpr explicit TLogApiContentContainerId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-content-container-" + std::to_string(value);
    }
};
