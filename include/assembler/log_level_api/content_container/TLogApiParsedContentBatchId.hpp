...
struct TLogApiParsedContentBatchId final {
    std::uint64_t value{0};

    constexpr TLogApiParsedContentBatchId() = default;
    constexpr explicit TLogApiParsedContentBatchId(std::uint64_t v) noexcept : value(v) {}

    [[nodiscard]] constexpr bool IsValid() const noexcept { return value != 0; }

    [[nodiscard]] std::string ToString() const {
        return "logapi-parsed-content-batch-" + std::to_string(value);
    }
};
