...
/**
 * @file TPortView.hpp
 * @brief Read-only bridge-visible host-side port view.
 *
 ...
 *
 * - diagnostic and binding-preflight projection for a host-side port;
 ...
 * - concrete port object ownership.
 *
...
{
    struct TPortView final
    {
        std::uint64_t port_id{0};
        TParticipantView participant{};
        TPortObligationProfile obligation_profile{};
        ...
        [[nodiscard]] static constexpr TPortView make(
            std::uint64_t id,
            TParticipantView participant_view,
            TPortObligationProfile profile,
        ...
        [[nodiscard]] static constexpr TPortView invalid() noexcept
        {
            return TPortView{};
        }
    };
