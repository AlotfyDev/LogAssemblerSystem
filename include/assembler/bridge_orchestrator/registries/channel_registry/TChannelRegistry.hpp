 ...
 * @brief Core bounded registry for ASCC channel declarations.
 *
 * @section ascc_comp_w01_registry_unification Registry Unification
 * This is the canonical registry path for `TChannelRegistry`.  The previous
 * implementation lived under `registries/optional_registries/typed`; that path
 * is now compatibility-only.
 *
 * @section ascc_non_responsibilities Non-Responsibilities
 * This registry catalogs channel declarations and diagnostics only.  It must
 * not execute channels, discover services, invoke endpoints, schedule work, or
 * behave as a broker.
 */
