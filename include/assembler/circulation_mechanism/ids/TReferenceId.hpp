#pragma once
    ...
    /*
        ...
        Current responsibility:
            Defines `TReferenceId` as a production-grade typed identity/locator
            atom in the Circulation Mechanism Engine.

        Semantic role:
            Identity of an access-reference artifact or reference-related diagnostic record.
        ...
        Valid usage:
            Use to correlate ingress/dispatch refs, invalidation reports, stale-ref diagnostics, and next-ref supply traces.

        Explicit invalid usage:
            Do not use as access permission, raw pointer, container id, slot id, or bridge protocol handle.
        ...
        Anti-collapse rule:
            `TReferenceId` may identify, correlate, or locate according to its
...
namespace circulation_mechanism {
    ...
    struct TReferenceId final
    {
        ...
        constexpr TReferenceId() noexcept = default;

        constexpr explicit TReferenceId(value_type input_value) noexcept
            : value(input_value)
        {
        }

        [[nodiscard]] static constexpr TReferenceId invalid() noexcept
        {
            return TReferenceId{k_invalid_value};
        }

        [[nodiscard]] static constexpr TReferenceId from_raw(value_type input_value) noexcept
        {
            return TReferenceId{input_value};
        }
        ...
        [[nodiscard]] friend constexpr bool operator==(TReferenceId lhs, TReferenceId rhs) noexcept
        {
    ...
    namespace std {
    template <>
    struct hash<assembler::circulation_mechanism::TReferenceId>
    {
        [[nodiscard]] std::size_t operator()(const assembler::circulation_mechanism::TReferenceId& input) const noexcept
        {
            return std::hash<assembler::circulation_mechanism::TReferenceId::value_type>{}(input.raw());
        }
    };
    } // namespace std
