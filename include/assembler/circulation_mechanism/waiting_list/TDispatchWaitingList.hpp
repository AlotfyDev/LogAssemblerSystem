...
#include "TDispatchWaitingListView.hpp"
#include "TDispatchWaitingListRegistrationReport.hpp"
...
class TDispatchWaitingList final {
public:
    ...
    [[nodiscard]] constexpr TDispatchWaitingListView view() const noexcept {
        return TDispatchWaitingListView{state_, size_, static_cast<std::uint32_t>(Capacity), sequence_, false};
    }

    constexpr TDispatchWaitingListRegistrationReport try_push(TWaitingListEntry entry) noexcept {
        if (is_locked(state_)) {
            return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::rejected_locked, size_};
        }
        if (!entry.is_valid()) {
            return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::rejected_invalid_index, size_};
        }
        if (!dispatch_waiting_list_eligible(entry.observed_container_state)) {
            return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::rejected_ineligible_state, size_};
        }
        if (contains_index(entry.container_index)) {
            return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::rejected_duplicate, size_};
        }
        if (full()) {
            return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::rejected_full, size_};
        }
        entry.fifo_sequence = ++sequence_;
        entries_[size_] = entry;
        ++size_;
        state_ = TWaitingListState::accepting_candidates;
        return TDispatchWaitingListRegistrationReport{entry, TWaitingListAdmissionStatus::accepted, size_};
    }
